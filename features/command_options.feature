Feature: Command Options
  ...
  As a player
  I want to customize rubysurvivor with options
  
  Background:
    Given no profile at "tmp"
  
  Scenario: Run ruby survivor in specified directory with -d option
    Given a profile named "Joe" on "beginner"
    When I copy fixture "walking_player.rb" to "tmp/rubysurvivor/joe-beginner/player.rb"
    And I run rubysurvivor with options "-d tmp/rubysurvivor/joe-beginner -t 0"
  ...
  Scenario: Skip user input with -s option
    Given a profile named "Joe" on "beginner"
    When I copy fixture "walking_player.rb" to "tmp/rubysurvivor/joe-beginner/player.rb"
    And I run rubysurvivor with options "-d tmp/rubysurvivor/joe-beginner -t 0 -s"
    Then I should see "current level"
    When I run rubysurvivor with options "-d tmp/rubysurvivor/joe-beginner -t 0"
    And I answer "y" to "next level"
    Then I should see "the updated README in the rubysurvivor/joe-beginner directory"
    When I run rubysurvivor with options "-d tmp/rubysurvivor/joe-beginner -t 0 -s"
    Then I should see "failed level 2"
  
  Scenario: Unable to practice level if not epic
    Given a profile named "Joe" on "beginner"
    When I copy fixture "walking_player.rb" to "tmp/rubysurvivor/joe-beginner/player.rb"
    And I run rubysurvivor with options "-d tmp/rubywarrior/joe-beginner -l 2"
  ...
  Scenario: Practice specific level when epic
    ...
    When I run rubysurvivor with options "-d tmp/rubysurvivor/bill-short -l 2"
