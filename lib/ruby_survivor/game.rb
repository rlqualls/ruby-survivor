module RubySurvivor
  class Game
    ...
    def start
      UI.puts "Welcome to Ruby Survivor"
    ...
    def request_next_level
      if !Config.skip_input? && (next_level.exists? ? UI.ask("Would you like to continue on to the next level?") : UI.ask("Would you like to continue on to epic mode?"))
        if next_level.exists?
          prepare_next_level
          UI.puts "See the updated README in the rubysurvivor/#{profile.directory_name} directory."
        else
          prepare_epic_mode
          UI.puts "Run rubysurvivor again to play epic mode."
        end
      else
    ...
    def profile_paths
      Dir[Config.path_prefix + '/rubysurvivor/**/.profile']
    end
