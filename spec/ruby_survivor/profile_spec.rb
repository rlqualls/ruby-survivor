...
describe RubySurvivor::Profile do
  before(:each) do
    @profile = RubySurvivor::Profile.new
  end
  
  it "should have survivor name" do
    @profile.survivor_name = 'Joe'
    @profile.survivor_name.should == 'Joe'
  end
  ...
  it "should decode with marshal + base64" do
    RubySurvivor::Profile.decode(@profile.encode).survivor_name.should == @profile.survivor_name
  end
  ...
  it "load should read file, decode and set player path" do
    ...
    RubySurvivor::Profile.expects(:decode).with('encoded_profile').returns(profile)
    RubySurvivor::Profile.load('path/to/.profile').should == profile
  end
