...
describe RubySurvivor::Units::Base do
  before(:each) do
  ...
  it "should print out line with name when speaking" do
    RubySurvivor::UI.expects(:puts_with_delay).with("Base foo")
    @unit.say "foo"
  end
  ...
  it "should add ability" do
    RubySurvivor::Abilities::Walk.expects(:new).with(@unit).returns('walk')
    @unit.add_abilities(:walk!)
